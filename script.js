let _sP=[],_cN={},_eE=new Set(),_sLO=[],_sML=[],_lPB64=null,_fPB64=null,_gE=[],_gTK=[],_gNS=0,_rFD=[];const _sU="https://bqemjroiegybdzksddkn.supabase.co",_sK="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxZW1qcm9pZWd5YmR6a3NkZGtuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODc1NzgwNDIsImV4cCI6MjEwMzE1NDA0Mn0.49gC204FPWSNxWYa6eZFBgWJgr7ZvFax5mqOM9lyGPo";

async function prepararRenombradoEquipos(){
    let fI=document.getElementById('fileInput'),fs=fI?Array.from(fI.files):[];if(!fs.length)return;
    _rFD=[];let eU=new Set();
    for(let f of fs){let t=await f.text();_rFD.push(t);t.split('\n').forEach(l=>{let m=l.match(/TeamName:\s*(.+?)\s+Rank:/i);if(m)eU.add(m[1].trim());});}
    let c=document.getElementById('listaEquiposInputs');if(!c)return;
    c.innerHTML='';Array.from(eU).forEach(eq=>{
...
function procesarConNombresPersonalizados(){
    let dR={};document.querySelectorAll('.input-nombre-editable').forEach(i=>{dR[i.getAttribute('data-original')]=i.value.trim()||i.getAttribute('data-original');});
    _xL(_rFD,dR);
}

async function procesarArchivosLog(){
    let fI=document.getElementById('fileInput');if(!fI||!fI.files.length){alert("Selecciona archivos .log o .txt");return;}
    let fs=Array.from(fI.files);_rFD=[];let ps=[];for(let i=0;i<fs.length;i++)ps.push(_lF(fs[i]));
    try{let rT=await Promise.all(ps);_rFD=rT;let dR={};document.querySelectorAll('.input-nombre-editable').forEach(i=>{dR[i.getAttribute('data-original')]=i.value.trim()||i.getAttribute('data-original');});_xL(_rFD,dR);await _sDB();}catch(e){console.error(e);alert("Error al procesar");}
}

function _lF(f){return new Promise((rs,rj)=>{let r=new FileReader();r.onload=e=>rs(e.target.result);r.onerror=e=>rj(e);r.readAsText(f);});}
...
function _xL(tA,rM){
    ...
    tA.forEach((t,i)=>{
        ...
        ls.forEach(l=>{
            ...
            if(tM){
                let rT=tM[1].trim(),nm=rM[rT]||rT;if(!eM[nm])eM[nm]={name:nm,totalScore:0,killScore:0,rankScore:0,salasPuntos:{},salasKills:{},salasBooyah:{},salasJugadas:0,booyahsCount:0};
            ...
            if(pM){let pN=pM[1].trim(),pK=parseInt(pM[2]);if(!jM[pN])jM[pN]={name:pN,kills:0};jM[pN].kills+=pK;}
        });
        if(sWT)rW.push({sala:i+1,team:sWT,points:sWTS,kills:sWKS});
        else rW.push({sala:i+1,team:"N/D",points:0,kills:0});
    });
    _gE=Object.values(eM);_gTK=Object.values(jM).sort((a,b)=>b.kills-a.kills);_gNS=nS;_sP=[new Map(Object.entries(eM))];
    window._rWGlobal = rW;
...
function cargarImagenLocal(e,tp){
    ...
    let r=new FileReader();r.onload=ev=>{if(tp==='logo')_lPB64=ev.target.result;else if(tp==='fondo')_fPB64=ev.target.result;renderizarResultadosFromState();};r.readAsDataURL(f);
}

function renderizarResultadosFromState(){if(!_gE.length)return;renderizarResultados(_gE,_gTK,_gNS);}
...
function renderizarResultados(eqs,tKs,nS){
    ...
    let eqO=[...eqs].map(eq=>{
        let tV=eq.totalScore;if(mC==='2')tV=eq.rankScore;else if(mC==='3')tV=eq.killScore;
        return{...eq,totalCalculado:tV};
    });
    if(mC==='1')eqO.sort((a,b)=>b.totalScore-a.totalScore);else if(mC==='2')eqO.sort((a,b)=>b.rankScore-a.rankScore);else if(mC==='3')eqO.sort((a,b)=>b.killScore-a.killScore);

    let tKL=[...tKs].slice(0,15),tM="TABLA GENERAL (ESTÁNDAR)";if(mC==='2')tM="TABLA SOLO POSICIÓN (RANKSCORE)";if(mC==='3')tM="TABLA SOLO KILLS";
    ...
    // TABLA PRINCIPAL GENERAL (top: 85px, altura para 15 equipos)
    ...
    eqO.map((eq,i)=>{
        let cFila=cF;if(i===0)cFila='#DCCC9C';else if(i===1)cFila='#959595';
    ...
    // ZONA INFERIOR 1: GANADORES DE BOOYAH POR SALA (top: 520px -> redistribuido con ~20px de separación)
    ...
    // ZONA INFERIOR 2: TOP 15 KILLERS MÁS LETALES EN 5 COLUMNAS (top: 660px -> redistribuido con espacio proporcionado abajo)
    ...
    tKL.map((tk,i)=>{let cP=cF;if(i===0)cP='#DCCC9C';return `<div style="font-size:0.7rem;background:rgba(18,19,23,0.85);padding:5px 6px;border-radius:5px;border:1px solid rgba(220,204,156,0.2);display:flex;justify-content:space-between;align-items:center;"><div><span style="color:${cP};font-weight:bold;margin-right:3px;">#${i+1}</span><span style="color:${cF};font-weight:bold;max-width:58px;display:inline-block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;vertical-align:middle;">${tk.name}</span></div><span style="color:#DCCC9C;font-weight:bold;font-size:0.75rem;">${tk.kills}</span></div>`;}).join('')+
    `</div></div></div><button onclick="descargar()" class="btn-generar" style="margin-top:20px;">DESCARGAR IMAGEN 4:5</button>`;
    
    document.getElementById('outputTablasLadoALado').innerHTML=h;
}

async function _sDB(){
    let sC=null;if(typeof supabase!=='undefined'&&supabase.createClient)sC=supabase.createClient(_sU,_sK);if(!sC)return;
    try{
        let ttl=document.getElementById('inputTituloTorneo')?document.getElementById('inputTituloTorneo').value:"ENTRENAMIENTO",
            jrn=document.getElementById('inputJornadaTorneo')?document.getElementById('inputJornadaTorneo').value:"PRÁCTICA",
            fch=new Date(),mod=document.getElementById('inputModerador')?document.getElementById('inputModerador').value:"",
            fI=document.getElementById('fileInput'),fs=fI?Array.from(fI.files):[],
            fN=`entreno_${fch.toISOString().slice(0,10)}_${ttl.replace(/[^a-zA-Z0-9]/g,'_').toLowerCase()}_${Date.now()}`;
        if(fs.length){for(let f of fs){let nL=f.name.replace(/[^a-zA-Z0-9_.-]/g,'_');await sC.storage.from('entrenamientos_logs').upload(`${fN}/${nL}`,f);}}
        let {data:sD,error:sE}=await sC.from('entrenamientos_sesiones').insert([{titulo:ttl,jornada:jrn,fecha:fch,moderador:mod,archivo_url:fN}]).select().single();if(sE)throw sE;
        let sId=sD.id,sRs=[];
        _gE.exports?.forEach?.(eq=>{}); // safety fallback
        _gE.forEach(eq=>{Object.keys(eq.salasPuntos).forEach(iS=>{sRs.push({sesion_id:sId,numero_sala:parseInt(iS)+1,equipo_nombre:eq.name,rank:0,kill_score:eq.killScore||0,rank_score:eq.rankScore||0,total_score:eq.salasPuntos[iS],es_booyah:Boolean(eq.salasBooyah&&eq.salasBooyah[iS])});});});
        if(sRs.length)await sC.from('salas_resultados').insert(sRs);
        let kRs=_gTK.map(tk=>({sesion_id:sId,jugador_nombre:tk.name,equipo_nombre:'Pumas Squad',kills:tk.kills}));
        if(kRs.length)await sC.from('top_killers').insert(kRs);
        alert("¡Sincronizado con la base de datos!");_cTP();
    }catch(e){console.error(e);}
}

async function _cTP(){
    let cT=document.getElementById('contenedorTopEquiposPublicos'),cK=document.getElementById('contenedorTopKillersPublicos');if(!cT&&!cK)return;
    let sC=null;if(typeof supabase!=='undefined'&&supabase.createClient)sC=supabase.createClient(_sU,_sK);if(!sC)return;
    try{
        let {data:sD,error:sE}=await sC.from('salas_resultados').select('equipo_nombre, total_score');
        if(!sE&&cT){
            let eM={};(sD||[]).forEach(r=>{if(!eM[r.equipo_nombre])eM[r.equipo_nombre]={name:r.equipo_nombre,totalScore:0};eM[r.equipo_nombre].totalScore+=(r.total_score||0);});
            let t10=Object.values(eM).sort((a,b)=>b.totalScore-a.totalScore).slice(0,10);
            cT.innerHTML=t10.length?t10.map((eq,i)=>{let cM=i===0?'var(--primary)':i===1?'#959595':i===2?'#cd7f32':'var(--gray)';return`<div style="background:rgba(18,19,23,0.6);padding:8px 12px;border-radius:6px;display:flex;justify-content:space-between;align-items:center;border:1px solid rgba(220,204,156,0.15);margin-bottom:6px;"><span style="font-weight:bold;font-size:0.85rem;"><span style="color:${cM};">#${i+1}</span> ${eq.name}</span><span style="font-size:0.80rem;color:var(--primary);font-family:'Orbitron';">${eq.totalScore} Pts</span></div>`;}).join(''):`<p style="color:var(--gray);font-size:0.85rem;">No hay registros de equipos.</p>`;
        }
        let {data:kD,error:kE}=await sC.from('top_killers').select('jugador_nombre, kills');
        if(!kE&&cK){
            let kM={};(kD||[]).forEach(r=>{if(!kM[r.jugador_nombre])kM[r.jugador_nombre]={name:r.jugador_nombre,kills:0};kM[r.jugador_nombre].kills+=(r.kills||0);});
            let t10k=Object.values(kM).sort((a,b)=>b.kills-a.kills).slice(0,10);
            cK.innerHTML=t10k.length?t10k.map((tk,i)=>{let cP=i===0?'var(--primary)':'var(--light)';return`<div style="background:rgba(18,19,23,0.6);padding:8px 12px;border-radius:6px;display:flex;justify-content:space-between;align-items:center;border:1px solid rgba(220,204,156,0.15);margin-bottom:6px;"><span style="font-weight:bold;font-size:0.85rem;"><span style="color:${cP};">#${i+1}</span> ${tk.name}</span><span style="font-size:0.80rem;color:var(--primary);font-family:'Orbitron';">${tk.kills} Kills</span></div>`;}).join(''):`<p style="color:var(--gray);font-size:0.85rem;">No hay registros de killers.</p>`;
        }
    }catch(e){console.error(e);}
}

function descargar(){
    let el=document.getElementById('tablaCaptura');if(!el)return;
    html2px?null:html2canvas(el,{scale:2,useCORS:true,backgroundColor:'#0a0a0c',logging:false}).then(canvas=>{
        let l=document.createElement('a');l.download='Tabla_Resultados_4_5.png';l.href=canvas.toDataURL('image/png');l.click();
    });
}
