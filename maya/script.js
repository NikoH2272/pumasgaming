...
function renderizarResultados(equipos, topKillers, roomWinners, numSalas) {
    ...
    let html = `
        <div id="tablaCaptura" style="width: 1200px; height: 580px; background-image: url('maya.png'); background-size: cover; position: relative; font-family: 'Rajdhani', sans-serif; color: #fff; padding: 15px;">
...
function procesarLogsGeneral(textsArray, renombresMap) {
    ...
    let html = `
        <div style="margin-bottom: 20px; text-align: center;">
            ...
            <div id="tablaCaptura1" style="width: 1200px; height: 580px; background-image: url('maya.png'); background-size: cover; position: relative; font-family: 'Rajdhani', sans-serif; color: #000; margin: 0 auto; text-align: left;">
                <div style="position: absolute; top: 50%; left: 2.5%; width: 95%; transform: translateY(-50%);">
                    <table style="width: 100%; border-collapse: collapse; font-size: 0.61rem; color: #000; table-layout: fixed;">
                        <thead>
                            <tr style="font-family: 'Orbitron'; font-weight: bold; font-size: 0.57rem;">
                                ${Array.from({length: 7}).map((_, bIdx) => {
                                    let colorFondo = bIdx % 2 === 0 ? 'rgba(200, 200, 200, 0.45)' : 'rgba(200, 230, 200, 0.45)';
                                    return `
                                        <th style="background: ${colorFondo}; padding: 3px 1px; text-align: center; width: 30px; border-bottom: 2px solid #000; ${bIdx > 0 ? 'border-left: 7px solid transparent; background-clip: padding-box;' : ''}">TOP</th>
                                        <th style="background: ${colorFondo}; padding: 3px 2px; text-align: left; width: 97px; border-bottom: 2px solid #000;">NOMBRE</th>
                                        <th style="background: ${colorFondo}; padding: 3px 1px; text-align: center; width: 35px; border-bottom: 2px solid #000; ${bIdx < 6 ? 'border-right: 7px solid transparent; background-clip: padding-box;' : ''}">PTS</th>
                                    `;
                                }).join('')}
                            </tr>
                        </thead>
...
// --- SECCIÓN NUEVA: RESULTADOS DETALLADOS POR SALA (USA mayasala.png) ---
...
function procesarLogsSala(textsArray) {
    ...
    // Ordenamos por puntuación total o kills según corresponda para la sala
    ...
    let html = `
        <div style="text-align: center;">
            ...
            <div id="tablaCapturaSala" style="width: 1080px; height: 1350px; background-image: url('mayasala.png'); background-size: cover; position: relative; font-family: 'Rajdhani', sans-serif; color: #000; margin: 0 auto; text-align: left;">
                
                <!-- TABLA CENTRADA (940x920) -->
                <div style="position: absolute; top: calc((1350px - 920px) / 2); left: calc((1080px - 940px) / 2); width: 940px; height: 920px; display: flex; align-items: center; justify-content: center;">
                    <table style="width: 100%; height: 100%; border-collapse: collapse; font-size: 1.3rem; color: #000; table-layout: fixed;">
                        <thead>
                            <tr style="font-family: 'Orbitron'; font-weight: bold; font-size: 1.2rem;">
                                <th style="background: rgba(200, 200, 200, 0.5); padding: 12px; text-align: center; width: 120px; border-bottom: 3px solid #000;">TOP</th>
                                <th style="background: rgba(200, 200, 200, 0.5); padding: 12px 20px; text-align: left; width: 460px; border-bottom: 3px solid #000;">NOMBRE</th>
                                <th style="background: rgba(200, 200, 200, 0.5); padding: 12px; text-align: center; width: 180px; border-bottom: 3px solid #000;">KILL</th>
                                <th style="background: rgba(200, 200, 200, 0.5); padding: 12px; text-align: center; width: 180px; border-bottom: 3px solid #000;">TOTAL</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${Array.from({length: 15}).map((_, i) => {
                                ...
                                let colorFondo = i % 2 === 0 ? 'rgba(200, 200, 200, 0.4)' : 'rgba(200, 230, 200, 0.4)';
                                let estiloCelda = `background: ${colorFondo}; padding: 10px 12px; border-bottom: 1px solid rgba(0,0,0,0.2);`;
